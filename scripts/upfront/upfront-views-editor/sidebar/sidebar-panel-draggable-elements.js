    ...
    define([
        ...
        return SidebarPanel.extend({
            ...
            update_sections: function () {
                ...
                var me = this,
                    section_have_els = 0,
                    first_section_with_elements = false,
                    active_section_with_elements = false;
                this.sections.each(function(section){
                    if ( section.elements && section.elements.size() > 0 ){
                        section_have_els++;
                        if ( ! first_section_with_elements )
                            first_section_with_elements = section;
                        if ( me.$el.find('.sidebar-panel-tabspane [data-target='+section.cid+']').hasClass('active') )
                            active_section_with_elements = section;
                    }
                    else {
                ...
                if ( active_section_with_elements ) {
                    this.$el.find('.sidebar-tab-content').hide();
                    this.$el.find('.sidebar-panel-content #'+active_section_with_elements.cid).show();
                }
                else if ( first_section_with_elements ) {
                    this.$el.find('.sidebar-panel-tab').removeClass('active');
                    this.$el.find('.sidebar-tab-content').hide();
                    this.$el.find('.sidebar-panel-tabspane [data-target='+first_section_with_elements.cid+']').addClass('active');
                    this.$el.find('.sidebar-panel-content #'+first_section_with_elements.cid).show();
                }
