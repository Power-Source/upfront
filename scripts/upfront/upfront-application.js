...
var LayoutEditorSubapplication = Subapplication.extend({
	...
	set_up_editor_interface: function () {
		...
		var _set_up_draggables = function () {
			var elements = {Element: [], DataElement: [], PluginElement: []},
			...
			_(app.Objects).each(function (obj, idx) {
				_.each(["Element", "DataElement", "PluginElement"], function(type_el){
					if (typeof obj[type_el] !== "function")
						return;
...
var Application = new (Backbone.Router.extend({
	...
	set_layout_up: function(layoutData){
		...
		var elementsPanel = this.sidebar.get_panel("elements"),
			layoutSection = elementsPanel && elementsPanel.get_section("layout");
		if (_.size(this.current_subapplication.Objects) && layoutSection && !layoutSection.elements.size()) {
			this.current_subapplication.set_up_editor_interface();
		}
