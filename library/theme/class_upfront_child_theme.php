...
abstract class Upfront_ChildTheme implements IUpfront_Server {
	...
	protected function up_update_nav_menu_item($menu_id, $db_id, $args, $menu_items, $parent_id = 0) {
	...
	private function getActiveIconFont() {
		...
		$fonts = json_decode((string)$this->get_theme_settings()->get('icon_fonts'), true);
		$active_font = false;
	...
	public function getLayoutProperties($properties = array()) {
		...
		if ($this->get_theme_settings()->get('global_regions')) {
			$properties[] = array(
				'name' => 'global_regions',
				'value' => json_decode((string)$this->get_theme_settings()->get('global_regions'))
			);
		}
	...
	public function getIconFonts($icon_fonts, $args) {
		...
		$icon_fonts_array = is_array( $icon_fonts ) ? $icon_fonts : json_decode((string)$icon_fonts);
	...
	public function getButtonPresets($button_presets, $args) {
		...
		return json_decode(is_string($button_presets) ? $button_presets : '', $as_array);
	}
	...
	public function getTabPresets($presets, $args) {
		...
		return json_decode(is_string($presets) ? $presets : '', $as_array);
	}
	...
	public function getAccordionPresets($presets, $args) {
		...
		$presetsArray = json_decode(is_string($presets) ? $presets : '', true);
	...
	public static function getPostImageVariants($image_variants = null, $args = null) {
		...
		return json_decode(is_string($image_variants) ? $image_variants : '');
	}
