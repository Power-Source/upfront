...
abstract class Upfront_Presets_Server extends Upfront_Server {
	...
	public function get_theme_presets() {
		...
		$theme_presets = false;
		if (is_object($settings) && $settings instanceof Upfront_Theme_Settings) {
			$presets = $settings->get($this->elementName . '_presets');
			$theme_presets = is_string($presets) && '' !== $presets
				? json_decode($presets, true)
				: false;
		}
	...
	public function get_presets_javascript_server() {
		...
		if(!is_array($presets)) {
			$presets = is_string($presets) && '' !== $presets
				? json_decode($presets, true)
				: array();
		}
	...
	private static function _get_l10n ($key=false) {
		$l10n = array(
			...
			'not_empty_label' => __('Preset-Name darf nicht leer sein.', 'upfront'),
			'special_character_label' => __('Preset-Name darf nur Zahlen, Buchstaben und Leerzeichen enthalten.', 'upfront'),
			...
			'border' => __('Rahmen', 'upfront'),
			'px' => __('px', 'upfront'),
			'type_element' => __('Typ Element:', 'upfront'),
			'typeface' => __('Schriftart:', 'upfront'),
			...
			'rounded_corners' => __('Abgerundete Ecken', 'upfront'),
			'typography' => __('Typografie', 'upfront'),
			'animate_hover_changes' => __('Animiert Zustandsänderungen', 'upfront'),
			'sec' => __('Sek', 'upfront'),
			...
			'posts_label' => __('Beitrag', 'upfront'),
			'reset_posts' => __('Zurücksetzen alle', 'upfront'),
			'default_label' => __('Standard', 'upfront'),
			'edit_preset_css' => __('Preset CSS bearbeiten', 'upfront'),
			'edit_preset_label' => __('Benutzerdefiniertes CSS', 'upfront'),
			'convert_style_to_preset' => __('Als Preset speichern', 'upfront'),
			'convert_preset_info' => __('Upfront 1.0 führt Presets ein, mit denen Du das Styling für jedes Element auf Deiner Webseite speichern und wiederverwenden kannst. Bevor Du dieses Element bearbeiten kannst, wähle eine der folgenden Optionen:', 'upfront'),
			'select_preset_info' => __('Bestehendes Preset auswählen (<strong>empfohlen</strong>):', 'upfront'),
			'save_as_preset_button_info' => __('Oder aktuellen Stil als neues Preset speichern:', 'upfront'),
			'preset_changed' => __('Preset geändert zu %s', 'upfront'),
			'preset_already_exist' => __('Preset %s existiert bereits, verwenden Sie einen anderen Namen!', 'upfront'),
			'preset_created' => __('Preset %s erfolgreich erstellt!', 'upfront'),
			'preset_reset' => __('Preset %s wurde zurückgesetzt!', 'upfront'),
			'default_overlay_title' => __('Standard-Preset bearbeiten', 'upfront'),
			'default_overlay_text' => __('<p>Bitte beachte, dass dieses Element das <strong>Standard-Preset</strong> verwendet. Änderungen an Presets wirken sich auf jedes Layout aus, in dem dieses Preset verwendet wird.</p><p>Um nur diese Instanz des Elements zu ändern, erstelle bitte ein neues Preset.</p>', 'upfront'),
			'default_overlay_button' => __('Standard-Preset bearbeiten', 'upfront')
		);
		return !empty($key)
