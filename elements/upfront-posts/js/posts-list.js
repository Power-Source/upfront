...
var PostsView = Upfront.Views.ObjectView.extend({
	...
	on_render: function () {
		...
		if (Upfront.Application.is_single() === false) {
			...
			if (pluginLayout) {
				this.$el.find(".upfront-object-content").empty().append('<div>Im Folgenden findest du Beispielinhalte für ' + pluginLayout.pluginName + '. Use it as a reference for styling.</div>' + pluginLayout.content);
				return;
			}
		}
