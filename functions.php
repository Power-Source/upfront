...
class Upfront {
	...
	private function _add_hooks () {
		...
		add_action('wp_ajax_upfront_log_client_error', array($this, 'log_client_error'));
	...
	public function add_edit_menu ($wp_admin_bar) {
		...
		$edit_url = home_url('/?editmode=true', is_ssl() ? "https" : null);
		$item = array(
			...
			'meta' => array(
				...
				'onclick' => 'if (!this.classList.contains("active")) { event.stopImmediatePropagation(); window.location.assign(this.href); return false; }',
				'html' => '<style>#wp-admin-bar-upfront-edit_layout > .ab-item { opacity: 1 !important; cursor: pointer !important; }</style>',
			),
		);
	...
	public function log_client_error () {
		if (!Upfront_Permissions::current(Upfront_Permissions::BOOT)) wp_die(0);
		$message = isset($_POST['message']) ? sanitize_text_field(wp_unslash($_POST['message'])) : '';
		if ($message) error_log('Upfront client error: ' . $message);
		wp_die(1);
	}
	...
	function inject_upfront_dependencies () {
		...
		if (empty($_GET['editmode']) && !apply_filters('upfront-load-editor-dependencies', false)) {
...
EOAdditivemarkup;
		...
		echo '<script type="text/javascript">
			(function () {
				function reportUpfrontError(message) {
					message = String(message || "Unknown client error");
					var loadingText = document.querySelector(".upfront-loading-text");
					if (loadingText) loadingText.textContent = "Upfront konnte nicht laden: " + message;
					if (window.console && console.error) console.error("Upfront konnte nicht laden:", message);
					if (window.jQuery) jQuery.post(' . json_encode(admin_url('admin-ajax.php')) . ', {
				...
				window.addEventListener("error", function (event) {
					reportUpfrontError(event.message + (event.filename ? " in " + event.filename + ":" + event.lineno : ""));
				});
				window.addEventListener("unhandledrejection", function (event) {
					reportUpfrontError(event.reason && event.reason.message ? event.reason.message : event.reason);
				});
				if (window.requirejs) {
					requirejs.onError = function (error) {
						var modules = error.requireModules && error.requireModules.length ? " [" + error.requireModules.join(", ") + "]" : "";
						reportUpfrontError((error.message || error.requireType || error) + modules);
					};
				}
				if (window.jQuery) {
